...
class _StatsScreenState extends State<StatsScreen> {
  ...
  // ★ ترتيب البرامج موحّد عبر التطبيق (مطابق لـ DatabaseService.getPrograms)
  List<String> _orderedPrograms = [];
  ...
  Future<void> _loadData() async {
    ...
    // ★ نحضر ترتيب البرامج من قاعدة البيانات (MIN(created_at)) — موحّد
    _orderedPrograms = await _dbService.getPrograms();
    _calculateStats();
  ...
  void _calculateStats() {
    // ★ نستخدم الترتيب الموحّد من قاعدة البيانات
    final programs = _orderedPrograms.isNotEmpty
        ? _orderedPrograms.map(_normalizeProgram).where((p) => p.isNotEmpty).toList()
        : _data
    ...
    _detailRows.add([
      'الإجمالي', _grandQuota, _grandDone, '$totalProgress%',
      grandOccCount, grandOccE, grandOccG, grandOccW, grandOccS,
    ]);
  }

  // ★ جديد v11.01: تعديل تسمية برنامج
  Future<void> _renameProgram(String oldName) async {
    final ctrl = TextEditingController(text: oldName);
    final newName = await showDialog<String>(
      context: context,
      builder: (c) => AlertDialog(
        title: const Text('✏️ تعديل تسمية البرنامج'),
        content: Column(mainAxisSize: MainAxisSize.min, children: [
          Text('التسمية الحالية: $oldName',
              style: const TextStyle(fontSize: 12, color: Colors.grey)),
          const SizedBox(height: 12),
          TextField(
            controller: ctrl,
            decoration: const InputDecoration(
              labelText: 'التسمية الجديدة',
    ...
    try {
      final affected = await _dbService.renameProgram(oldName, newName);
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text('✅ تم تحديث $affected سجل: "$oldName" → "$newName"'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          // ★ زر إدارة البرامج (تعديل تسمية)
          PopupMenuButton<String>(
            icon: const Icon(Icons.edit_note),
            tooltip: 'تعديل تسمية برنامج',
            onSelected: _renameProgram,
            itemBuilder: (c) => _orderedPrograms
  ...
  Widget _buildMainTable() {
    ...
    return DataTable(
      ...
      rows: _mainRows.map((row) {
        ...
        return DataRow(
          color: isTotal ? WidgetStateProperty.all(const Color(0xFFE0F2FE)) : null,
          cells: [
