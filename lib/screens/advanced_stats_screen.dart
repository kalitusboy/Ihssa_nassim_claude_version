...
class _AdvancedStatsScreenState extends State<AdvancedStatsScreen>
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(
            ...
            onPressed: () => kIsWeb ? _exportWeb() : _export(context),
            tooltip: 'تصدير CSV',
          ),
        ],
        bottom: TabBar(
  ...
  // ── تصدير CSV على الويب ────────────────────────
  Future<void> _exportWeb() async {
    final url = PlatformService().exportStatsCsvUrl();
    if (url != null) {
      await openUrl(url);
    }
  }

  // ── تصدير CSV على الهاتف ───────────────────────
  Future<void> _export(BuildContext ctx) async {
    if (kIsWeb) { await _exportWeb(); return; }
    try {
